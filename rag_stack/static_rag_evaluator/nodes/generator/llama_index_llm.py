...
class LlamaIndexLLM(BaseGenerator):
	def __init__(self, project_dir: str, model: str, batch: int = 100, *args, **kwargs):
		"""
		...
		super().__init__(project_dir=project_dir, model=model)
		self.batch = batch
		...
		# Pass model name as 'model' kwarg to LlamaIndex LLM class
		kwargs.setdefault("model", self.model)
		self.llm_instance: BaseLLM = llm_class(**pop_params(llm_class.__init__, kwargs))

	def __del__(self):
		super().__del__()
		close_llm_async_client(self.llm_instance)
