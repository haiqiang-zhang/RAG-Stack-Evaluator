...
_COMPONENT_TELEMETRY_WORKER = (
	"rag_stack.vllm_instrumentation.llm_sim.calibration.component_telemetry."
...
def _fit_request_to_context(
	*,
	...
	if len(str(prompt)) + requested <= limit:
		return requested, None
	try:
		from rag_stack.static_rag_evaluator.recording import count_tokens_batch
...
class VllmSubprocess:
	...
	def __init__(self, key: VllmStartupKey):
		...
		from rag_stack.static_rag_evaluator.measured.gpu_mem import effective_util
