...
class MeasuredEvaluator:
	...
	def _build_service_stages(
		...
		for node_line_name, nodes in node_lines.items():
			for node in nodes:
				...
				from rag_stack.static_rag_evaluator.measured.aux_process import (
					AuxProcessStage,
	...
	def _inject_cache_and_devices(
		...
		`rag_stack.static_rag_evaluator.measured.cache.get_current()`.
		...
		"""
		from rag_stack.static_rag_evaluator.measured.cache import set_current as _set_current_cache
		from rag_stack.search_space.placement import GPU_STAGES
		from rag_stack.system_layout import (
			stage_device,
			stage_devices,
		...
		# the right card.
		from rag_stack.static_rag_evaluator.embedding.base import set_embedding_device
