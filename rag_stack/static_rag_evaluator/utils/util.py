...
def fetch_contents(
	...
	# Build once per corpus DataFrame and reuse for both sequential and ReAct
	# retrieval paths. The old implementation scanned the full corpus for every
	# returned id, which made large corpora such as triviaqa pay O(N * top_k)
	# Python/pandas overhead inside measured latency.
	lookup = _fetch_lookup(corpus_data, "doc_id", column_name)

	def fetch_contents_pure(
		ids: List[str], corpus_data: pd.DataFrame, column_name: str
	):
		return list(map(lambda x: fetch_one_content(corpus_data, x, column_name, lookup=lookup), ids))
...
def prewarm_fetch_lookup(
	corpus_data: pd.DataFrame,
	id_column_name: str = "doc_id",
	column_name: str = "contents",
) -> None:
	"""Build the id->column lookup at ASSEMBLY time.

	Nodes that consult a corpus-sized lookup per request must pay the build
	(minutes of pandas on an 8.8M-row corpus) in their __init__, never inside
	the measured serving clock — a lazy first build at serving start stalled
	the whole closed loop for 8+ minutes (msmarco smac s44/eval_0009)."""
	_fetch_lookup(corpus_data, id_column_name, column_name)


_FETCH_LOOKUP_BUILD_LOCK = threading.Lock()
...
def _fetch_lookup(
	...
	cache = corpus_data.attrs.setdefault("_rag_stack_fetch_lookup", {})
	key = (id_column_name, column_name)
	built = cache.get(key)
	if built is not None:
		return built
	# Single-builder lock: at serving start hundreds of concurrent requests
	# hit an empty cache from executor threads, and WITHOUT the lock every
	# thread built its own copy of the corpus-sized dict simultaneously —
	# GIL-serialized minutes of pandas that starved the event loop to one
	# tick per ~5-7 s (the s44/eval_0009 cold-start stall, convicted by
	# in-process stack dumps). Waiters block on the lock instead of burning
	# the GIL; exactly one build happens per (corpus, column) pair.
	with _FETCH_LOOKUP_BUILD_LOCK:
		built = cache.get(key)
		if built is not None:
			return built
		built = _build_fetch_lookup(corpus_data, id_column_name, column_name)
		cache[key] = built
	return built


def _build_fetch_lookup(
	corpus_data: pd.DataFrame,
	id_column_name: str,
	column_name: str,
) -> Dict[Any, Any]:
	if id_column_name not in corpus_data.columns:
		raise ValueError(f"{id_column_name!r} not found in corpus_data.")
	if column_name not in corpus_data.columns:
		raise ValueError(f"{column_name!r} not found in corpus_data.")
	return (
		corpus_data[[id_column_name, column_name]]
		.drop_duplicates(subset=[id_column_name], keep="first")
		.set_index(id_column_name)[column_name]
		.to_dict()
	)
...
def fetch_one_content(
	...
	if isinstance(id_, str):
		if id_ in ["", ""]:
			return None
		table = lookup if lookup is not None else _fetch_lookup(
			corpus_data, id_column_name, column_name
		)
		if id_ not in table:
...
def close_llm_async_client(llm) -> None:
	"""Close the cached httpx AsyncClient on a LlamaIndex LLM before GC.

	LlamaIndex LLM classes cache an ``_aclient`` (httpx AsyncClient).
	If the event loop is closed before GC collects the LLM, ``__del__``
	raises ``RuntimeError('Event loop is closed')``.
	See: https://github.com/openai/openai-python/issues/1254
	"""
	aclient = getattr(llm, "_aclient", None)
	if aclient is None:
		return
	try:
		loop = get_event_loop()
