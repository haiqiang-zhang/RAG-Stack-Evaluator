...
``rag_stack.static_rag_evaluator.chunk_cache``) so that everything dataset-shaped
...
class DatasetManager:
    ...
    def _chunk_raw_at_init(self, raw_path: str, qa_path: str) -> str:
        ...
        """
        from rag_stack.static_rag_evaluator.chunk_cache import get_or_build
        ...
        if not self._config:
            from rag_stack.static_rag_evaluator.chunk_cache import _run_chunker
    ...
    def activate(self, view: CorpusView) -> pd.DataFrame:
        ...
        """
        from rag_stack.static_rag_evaluator.chunk_cache import NO_OP_HASH
