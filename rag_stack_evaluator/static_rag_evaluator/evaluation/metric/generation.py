...
from rag_stack_evaluator.static_rag_evaluator.embedding.base import embedding_models
...
# ---------------------------------------------------------------------------
# SQuAD-style EM + token F1 (Rajpurkar et al. 2016 normalization).
# Same recipe used by HotpotQA, NaturalQuestions, TriviaQA, FlashRAG —
# the standard GT-anchored partial-credit baseline that reviewers expect
# alongside LLM-judge metrics.
# ---------------------------------------------------------------------------
...
def _normalize_answer(text: str) -> str:
	"""SQuAD canonical normalization: lowercase, strip punctuation,
	strip English articles (a / an / the), collapse whitespace."""
	text = text.lower()
...
def _token_f1(pred: str, gt: str) -> float:
	...
	# Edge case: both empty → exact match → 1.0; one empty → 0.0.
...
@autorag_metric_loop(fields_to_check=["generation_gt", "generated_texts"])
def em(metric_inputs: List[MetricInput], **kwargs) -> List[float]:
	"""SQuAD-style Exact Match. 1.0 if the normalized prediction equals
	any normalized ground-truth answer, else 0.0. Normalization: lower-case,
	strip punctuation + English articles, collapse whitespace.
	"""
	results = []
	for mi in metric_inputs:
		pred_norm = _normalize_answer(mi.generated_texts)
		gts = mi.generation_gt if isinstance(mi.generation_gt, list) else [mi.generation_gt]
		score = max((1.0 if _normalize_answer(g) == pred_norm else 0.0) for g in gts)
		results.append(score)
	return results


@autorag_metric_loop(fields_to_check=["generation_gt", "generated_texts"])
def f1(metric_inputs: List[MetricInput], **kwargs) -> List[float]:
	"""SQuAD-style token-level F1. Per-token bag overlap between prediction
	and ground truth, scored as harmonic mean of token precision and recall;
	final score is max over multiple GT answers if provided.
	"""
	results = []
	for mi in metric_inputs:
		gts = mi.generation_gt if isinstance(mi.generation_gt, list) else [mi.generation_gt]
		score = max(_token_f1(mi.generated_texts, g) for g in gts)
		results.append(score)
	return results
