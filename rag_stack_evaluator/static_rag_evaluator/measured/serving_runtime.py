...
from rag_stack_evaluator.static_rag_evaluator.measured import performance as perf_mod
...
class MeasuredServingRuntime:
	...
	async def _run_closed_loop_saturated(
		...
		if recorder is None:
			...
			from rag_stack_evaluator.static_rag_evaluator.recording import get_current_recorder
	...
	async def _run_react_request(self, state: RequestState) -> None:
		from rag_stack_evaluator.static_rag_evaluator.agentic_react import (
			_FINISH_RE,
			_REACT_PROMPT,
			_SEARCH_RE,
			_STOP_TOKENS,
...
def _record_generate(previous: pd.DataFrame, result: pd.DataFrame, params: Dict[str, Any]) -> None:
	from rag_stack_evaluator.static_rag_evaluator import recording as rec
...
def _query_expansion_prompt_and_parser(
	...
	if module_name == "HyDE":
		from rag_stack_evaluator.static_rag_evaluator.nodes.queryexpansion.hyde import hyde_prompt
	...
	if module_name == "QueryDecompose":
		from rag_stack_evaluator.static_rag_evaluator.nodes.queryexpansion.query_decompose import (
			decompose_prompt,
			get_query_decompose,
		)
