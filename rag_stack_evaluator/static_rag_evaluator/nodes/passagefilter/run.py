...
from rag_stack_evaluator.static_rag_evaluator.nodes.retrieval.run_util import evaluate_retrieval_node
...
from rag_stack_evaluator.static_rag_evaluator.utils.util import to_list, apply_recursive
...
def run_passage_filter_node(
	...
	"""
	Run the single passage filter module supplied by the upstream sampler.

	Greedy multi-module selection has been removed. Exactly one module is expected.
	"""
	if len(modules) != 1:
		raise ValueError(
			f"passage_filter expects exactly one module after sampling, "
			f"got {len(modules)}"
		)
	module = modules[0]
	...
	save_dir = os.path.join(node_line_dir, "passage_filter")
	...
	summary_df = pd.DataFrame(
		{
			...
			**{
				f"passage_filter_{metric}": [result[metric].mean()]
