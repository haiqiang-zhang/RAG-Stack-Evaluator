...
from rag_stack_evaluator.static_rag_evaluator.evaluation.retrieval import RETRIEVAL_METRIC_FUNC_DICT
...
from rag_stack_evaluator.static_rag_evaluator.utils.util import apply_recursive, to_list
...
def run_hybrid_retrieval_node(
	...
	"""
	Run the single hybrid retrieval module supplied by the upstream sampler.

	Greedy multi-module selection has been removed. Exactly one module is expected.
	Internal weight sweeping in hybrid_rrf/hybrid_cc has also been removed — the
	weight is sampled by Ax and passed in module_param.
	"""
	if len(modules) != 1:
		raise ValueError(
			f"hybrid_retrieval expects exactly one module after sampling, "
			f"got {len(modules)}"
		)
	module = modules[0]
	...
	save_dir = os.path.join(node_line_dir, "hybrid_retrieval")
...
def evaluate_retrieval_node(
	...
	"""Evaluate hybrid retrieval result with the given metrics."""
