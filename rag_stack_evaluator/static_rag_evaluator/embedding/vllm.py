...
class VllmEmbedding(MultiModalEmbedding):
	...
	def close():
		...
		# Best-effort GPU cleanup at process exit. If CUDA context is already
		# torn down or memory is exhausted, the sync/empty_cache calls raise
		# `torch.AcceleratorError`. Swallow them — Python's atexit prints the
		# raw traceback to stderr otherwise, polluting downstream logs. The
		# OS reclaims any leaked GPU memory on process exit regardless.
		try:
