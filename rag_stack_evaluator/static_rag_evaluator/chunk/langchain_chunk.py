...
from rag_stack_evaluator.static_rag_evaluator.chunk.base import chunker_node, add_file_name
from rag_stack_evaluator.static_rag_evaluator.chunk.utils import add_essential_metadata, get_start_end_idx
...
def langchain_chunk_pure(
	...
	# make doc_id — CONTENT-DERIVED, not random (07-05): the global faiss
	# index cache shares indexes across runs keyed by chunk_hash; the doc_ids
	# embedded in an index must therefore be reproducible by ANY re-chunk of
	# the same content. sha1(chunk text) + position gives a stable identity
	# (position disambiguates identical chunks); uuid4 broke every cross-run
	# index reuse with 'doc_id not found in corpus_data'.
	import hashlib
	doc_id = [
		hashlib.sha1(f"{i}\x00{doc.page_content}".encode("utf-8", "ignore")).hexdigest()
