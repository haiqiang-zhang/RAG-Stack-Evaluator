...
from rag_stack_evaluator.static_rag_evaluator.embedding.base import EmbeddingModel
...
def chunker_node(func):
	...
	def wrapper(
		...
		# Raw corpus carries the document text in the canonical ``contents``
		# column (single corpus schema {doc_id, contents}; legacy ``texts`` is
		# reverse-aliased to ``contents`` upstream in chunk_cache).
		...
		if func.__name__ in ["llama_index_chunk", "langchain_chunk"]:
			...
			raise ValueError(f"Unsupported component: {func.__name__}")

	return wrapper


def make_metadata_list(parsed_result: pd.DataFrame) -> List[Dict[str, str]]:
	metadata_list = [{} for _ in range(len(parsed_result["contents"]))]
...
def __get_chunk_instance(component: str, chunk_method: str, **kwargs):
	...
	if chunk_method in embedding_available_methods:
		...
		embed_model = get_embedding_model(embed_model_str, component)
